...
std::vector<T> ReadMetaFile(const std::string& meta_path) {
  ...
  while (true) {
    ...
    if (!ifs || ifs.gcount() != static_cast<std::streamsize>(sizeof(header))) {
      LOG(ERROR) << "truncated length header at record " << record_no << " of "
                 << meta_path;
      break;
    }
    ...
    if (size > 0) {
      ...
      if (!ifs || ifs.gcount() != static_cast<std::streamsize>(size)) {
        LOG(ERROR) << "truncated payload at record " << record_no
                   << " (expected " << size << " bytes) of " << meta_path;
        break;
      }
    }
    T msg;
    if (!msg.ParseFromString(buf)) {
      LOG(WARNING) << "failed to parse record " << record_no << " of "
                   << meta_path;
    } else {
...
template <typename T>
bool WriteMetaFile(const std::string& meta_path,
  ...
  for (const auto& msg : messages) {
    ...
    if (buf.size() > std::numeric_limits<uint32_t>::max()) {
      LOG(ERROR) << "message too large to serialize (" << buf.size()
                 << " bytes)";
      return false;
    }

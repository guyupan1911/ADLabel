...
class BaseCamera {
  public:
    ...
    virtual bool Project(const Eigen::Matrix3d& R,
    ...
    virtual void ImageToNormalized(double x, double y, double* u, double* v) const = 0;

    // Undistorted normalized camera plane coordinate -> distorted pixel.
    virtual void NormalizedToImage(double u, double v, double* x, double* y) const = 0;

  protected:
    BaseCamera(CameraType type, size_t width, size_t height, std::vector<double> params);

    // Computes distortion offset. Distorted normalized coordinate is
    // (u + delta_u, v + delta_v).
    virtual void Distort(double u, double v, double* delta_u, double* delta_v) const = 0;

    void IterativeUndistort(double distorted_u,
                            double distorted_v,
                            double* u,
                            double* v) const;
...
class OpenCVPinholeCamera : public BaseCamera {
  public:
    OpenCVPinholeCamera();
    OpenCVPinholeCamera(size_t width,
                        size_t height,
    ...
    void ImageToNormalized(double x, double y, double* u, double* v) const override;
    void NormalizedToImage(double u, double v, double* x, double* y) const override;

  protected:
    void Distort(double u, double v, double* delta_u, double* delta_v) const override;
};

class OpenCVFisheyeCamera : public BaseCamera {
  public:
    OpenCVFisheyeCamera();
    OpenCVFisheyeCamera(size_t width,
                        size_t height,
                        const Eigen::Matrix3d& K,
                        const std::vector<double>& distortion_coefficients);

...
void LidarLosslessMapNode::Reset() {
    matrix_.Reset();
}
...
void LidarLosslessMapNode::GetIntensityImage(cv::Mat* image, size_t min_samples) const {
    *image = cv::Mat::zeros(
            static_cast<int>(frame_.rows), static_cast<int>(frame_.cols), CV_8UC1);
    matrix_.ForEachOccupied([&](unsigned int r, unsigned int c, const LosslessMapCell& cell) {
        if (cell.GetCount() < min_samples) return;
        image->at<unsigned char>(static_cast<int>(r), static_cast<int>(c)) = cell.GetValue();
    });
}

void LidarLosslessMapNode::GetAltitudeImage(cv::Mat* image, size_t min_samples) const {
    *image = cv::Mat(static_cast<int>(frame_.rows),
                     static_cast<int>(frame_.cols),
                     CV_32FC1,

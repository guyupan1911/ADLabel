...
DEFINE_string(camera_metadata, "", "camera metadata path, relative to data_root or absolute");
DEFINE_string(data_root, "", "root directory for sensor data");
DEFINE_string(output_dir, "", "directory to save projected images");
DEFINE_int64(max_time_diff_ms, 10, "maximum lidar-camera timestamp difference in milliseconds");
DEFINE_double(max_render_depth_m, 80.0, "maximum depth used by color rendering");
DEFINE_int32(point_radius, 1, "projected point radius in pixels");
DEFINE_uint64(max_frames, 0, "maximum matched frame pairs to process, 0 means all");
DEFINE_string(depth_metric,
              "euclidean",
...
DEFINE_bool(enable_motion_compensation, true,
            "enable motion compensation using LIO pose interpolation for timestamp differences");
...
bool IsProcessableCameraFrame(const Frame& frame) {
    if (!frame.has_camera_image_uri() || frame.camera_image_uri().empty()) {
        LOG(WARNING) << "skip camera frame without camera_image_uri: " << frame.fid();
        return false;
    }
    if (!frame.has_sensor_to_imu_extrinsic()) {
...
int Run() {
    ...
    CHECK_GE(FLAGS_max_time_diff_ms, 0) << "--max_time_diff_ms must be non-negative";
    CHECK_GT(FLAGS_max_render_depth_m, 0.0) << "--max_render_depth_m must be positive";
    ...
    CHECK(!lidar_frames.empty()) << "no lidar frames loaded from "
                                 << FLAGS_lidar_metadata;
    CHECK(!camera_frames.empty()) << "no camera frames loaded from "
                                  << FLAGS_camera_metadata;
    ...
    for (size_t pair_index = 0; pair_index < aligned_frames.size(); ++pair_index) {
        ...
        if (camera->Width() != static_cast<size_t>(image.cols) ||
            ...
            LOG(WARNING) << "camera calibration size " << camera->Width()
                         << "x" << camera->Height()
                         << " differs from image size " << image.cols
                         << "x" << image.rows;
        }
        ...
        if (FLAGS_enable_motion_compensation &&
            ...
            if (ComputeMotionCompensation(lio_pose_interpolator,
                                          lidar_frame.timestamp_ns(),
                ...
                const Eigen::AngleAxisd rotation_diff(
                        T_motion_compensation.rotation());
                LOG(INFO) << "motion compensation applied, time_diff_ms="
                          << (camera_frame.timestamp_ns() -
                              lidar_frame.timestamp_ns()) /
                                     1e6
                          << ", translation_norm_m="
                          << translation_diff.norm()
                          << ", rotation_angle_deg="
                          << rotation_diff.angle() * 180.0 / M_PI;
            }
        }

...
int Run() {
  ...
  for (const auto& frame : frames) {
    ...
    const Eigen::Vector3d pose_enu =
        EcefToEnu(pose_translation_ecef, enu_origin_ecef, ecef_to_enu_rotation);
  ...
  for (size_t i = 0; i < local_frames.size(); ++i) {
    if (FLAGS_distance > 0.0 && !current_segment.empty()) {
      const Eigen::Vector2d& last_xy =
          local_frames[current_segment.back()].origin_xy;
  ...
  LOG(INFO) << "loaded " << frames.size()
            ...
            << ", segments=" << frame_segments.size() << ", origin timestamp="
            << local_frames.front().frame.timestamp_ns();

  for (size_t segment_index = 0; segment_index < frame_segments.size();
       ++segment_index) {
    ...
    for (const size_t frame_index : segment) {
      ...
      for (const auto& point : lidar_frame_data.raw_cloud->points) {
        ...
        node.SetValue(p_enu, frame.sensor_name(),
    ...
    if (processed_frames == 0) {
      LOG(ERROR) << "no lidar frames were processed in segment "
                 << segment_index;
      return 1;
    }

...
DEFINE_string(output_dir, "", "directory to save fused topdown intensity image");
...
int Run() {
    ...
    CHECK(!local_frames.empty()) << "no processable lidar frames in any data_root";
    ...
    for (const auto& local_frame : local_frames) {
        ...
        if (!GenerateLidarFrameData(local_frame.frame,
                                    &lidar_frame_data,
                                    local_frame.data_reader)) {
            LOG(ERROR) << "failed to generate lidar frame data: " << local_frame.frame.fid();
            continue;
        }
    ...
    const std::filesystem::path output_path = output_dir / "topdown_intensity_image.png";

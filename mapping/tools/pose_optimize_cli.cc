...
DEFINE_string(lidar_metadata, "", "Input lidar metadata file with lio_pose_3d.");
DEFINE_string(localization_bin, "", "Length-prefixed LocalizationEstimation bin file.");
DEFINE_string(output_eval_csv,
              "data/pose_optimization_eval.csv",
              "CSV containing timestamp-aligned gnss, aligned_lio, and optimized poses.");
DEFINE_string(output_graph_nodes_csv,
              "data/pose_graph_nodes.csv",
              "CSV containing graph nodes for visualization.");
DEFINE_string(output_graph_factors_csv,
              "data/pose_graph_factors.csv",
              "CSV containing graph factors for visualization.");

DEFINE_double(max_gnss_gap_sec, 0.2, "Max allowed time distance to GNSS interpolation bounds.");
DEFINE_double(lio_translation_sigma, 0.02, "LIO between-factor translation sigma in meters.");
DEFINE_double(lio_rotation_sigma_rad, 0.003, "LIO between-factor rotation sigma in radians.");
...
DEFINE_double(min_gnss_factor_distance_m,
              20.0,
              "Minimum accumulated LIO trajectory distance between RTK fixed GNSS factors in "
              "meters. Set to 0 to disable distance downsampling.");
DEFINE_bool(use_local_coordinate,
            true,
            "Optimize in a local coordinate frame whose origin is the first RTK fixed GNSS pose.");
DEFINE_bool(add_first_prior, false, "Add a first-pose prior for ablation experiments.");
DEFINE_double(first_prior_translation_sigma, 0.05, "First pose prior translation sigma in meters.");
DEFINE_double(first_prior_rotation_sigma_rad, 0.02, "First pose prior rotation sigma in radians.");
...
bool InterpolateRtkFixedGnssPose(const std::vector<LocalizationEstimation>& gnss_msgs,
                                 double timestamp_sec,
                                 double max_gap_sec,
                                 gtsam::Pose3* pose) {
    if (gnss_msgs.empty()) {
...
double YawOfPose(const gtsam::Pose3& pose) {
...
RpeMetrics ComputeRpe(const gtsam::Pose3& reference_from,
    ...
    metrics.yaw_error_rad = WrapAngle(YawOfPose(evaluated_relative) - YawOfPose(reference_relative));
    return metrics;
}
...
RpeMetrics ComputeOptimizedRpe(const std::vector<OptimizerNode>& nodes,
    ...
    return ComputeRpe(nodes[from].aligned_lio_pose,
                      nodes[to].aligned_lio_pose,
...
RpeMetrics ComputeOptimizedWindowRpe(const std::vector<OptimizerNode>& nodes,
    ...
    auto iter = std::lower_bound(
            nodes.begin(),
            nodes.begin() + static_cast<std::ptrdiff_t>(to),
            target_ts,
    ...
    if (iter != nodes.begin()) {
        ...
        if (std::abs(prev->timestamp_sec - target_ts) < std::abs(iter->timestamp_sec - target_ts)) {
            iter = prev;
        }
    }
    return ComputeOptimizedRpe(nodes, result, static_cast<std::size_t>(iter - nodes.begin()), to);
}

void WriteRpeFields(std::ostream& os, const RpeMetrics& metrics) {
    os << metrics.valid << "," << metrics.translation_error_m << "," << metrics.rotation_error_rad
...
bool WriteEvaluationCsv(const std::string& path,
    ...
    ofs << "timestamp_sec,"
        << "gnss_x,gnss_y,gnss_z,gnss_qx,gnss_qy,gnss_qz,gnss_qw,gnss_roll,gnss_pitch,"
           "gnss_yaw,"
        << "aligned_lio_x,aligned_lio_y,aligned_lio_z,aligned_lio_qx,aligned_lio_qy,"
           "aligned_lio_qz,aligned_lio_qw,aligned_lio_roll,aligned_lio_pitch,"
           "aligned_lio_yaw,"
        << "optimized_x,optimized_y,optimized_z,optimized_qx,optimized_qy,optimized_qz,"
           "optimized_qw,optimized_roll,optimized_pitch,optimized_yaw,"
        << "aligned_lio_pos_error_m,optimized_pos_error_m,aligned_lio_yaw_error_rad,"
           "optimized_yaw_error_rad,"
        << "aligned_lio_error_x_m,aligned_lio_error_y_m,aligned_lio_error_z_m,"
           "aligned_lio_error_roll_rad,aligned_lio_error_pitch_rad,aligned_lio_error_yaw_rad,"
        << "optimized_error_x_m,optimized_error_y_m,optimized_error_z_m,"
           "optimized_error_roll_rad,optimized_error_pitch_rad,optimized_error_yaw_rad,"
    ...
    for (std::size_t i = 0; i < nodes.size(); ++i) {
        ...
        ofs << "," << PositionError(aligned_lio, gnss) << "," << PositionError(optimized, gnss)
            ...
            << WrapAngle(aligned_rpy.z() - gnss_rpy.z()) << "," << opt_t.x() - gnss_t.x()
            << "," << opt_t.y() - gnss_t.y() << "," << opt_t.z() - gnss_t.z() << ","
            << WrapAngle(opt_rpy.x() - gnss_rpy.x()) << ","
            << WrapAngle(opt_rpy.y() - gnss_rpy.y()) << ","
            << WrapAngle(opt_rpy.z() - gnss_rpy.z()) << "," << nodes[i].has_gnss_factor << ","
...
bool WriteGraphNodesCsv(const std::string& path,
    ...
    for (std::size_t i = 0; i < nodes.size(); ++i) {
        ...
        ofs << i << "," << nodes[i].timestamp_sec << "," << aligned.x() << "," << aligned.y()
            << "," << aligned.z() << "," << optimized.x() << "," << optimized.y() << ","
            << optimized.z() << "," << gnss.x() << "," << gnss.y() << "," << gnss.z() << ","
    ...
    while (start <= dir.size()) {
        ...
        if (!part.empty()) {
            ...
            if (mkdir(current.c_str(), 0755) != 0 && errno != EEXIST) {
                LOG(ERROR) << "failed to create directory " << current << ": " << strerror(errno);
                return false;
            }
        }
...
private:
    bool LoadInputs() {
        ...
        if (gnss_msgs_.empty()) {
            LOG(ERROR) << "no localization messages loaded from " << FLAGS_localization_bin;
            return false;
        }
        std::sort(gnss_msgs_.begin(), gnss_msgs_.end(), [](const auto& a, const auto& b) {
            return GnssTimestampSec(a) < GnssTimestampSec(b);
        });
        LOG(INFO) << "loaded frames=" << frames_.size() << ", gnss=" << gnss_msgs_.size();
        return true;
    }
    ...
    bool BuildNodes() {
        ...
        double last_gnss_factor_distance_m = -std::numeric_limits<double>::infinity();
        ...
        for (std::size_t i = 0; i < frames_.size(); ++i) {
            ...
            if (!InterpolateGnssPose(gnss_msgs_, ts, FLAGS_max_gnss_gap_sec, &gnss_pose)) {
        ...
        LOG(INFO) << "optimizer nodes=" << nodes_.size()
                  ...
                  << ", skipped_non_fixed_gnss_factor=" << skipped_non_fixed_gnss_factor
                  << ", skipped_gnss_factor_by_distance=" << skipped_gnss_factor_by_distance
                  << ", min_gnss_factor_distance_m=" << FLAGS_min_gnss_factor_distance_m;
        if (nodes_.size() < 2) {
            LOG(ERROR) << "need at least 2 nodes";
            return false;
        }
        auto anchor_iter = std::find_if(nodes_.begin(), nodes_.end(), [](const auto& node) {
            return node.has_gnss_factor;
        });
        if (anchor_iter == nodes_.end()) {
            LOG(ERROR) << "no RTK fixed GNSS poses found";
            return false;
        }
        const gtsam::Pose3 T_global_lio = anchor_iter->gnss_pose * anchor_iter->lio_pose.inverse();
        local_to_global_ = FLAGS_use_local_coordinate
                                   ? gtsam::Pose3(gtsam::Rot3(),
                                                  anchor_iter->gnss_pose.translation())
                                   : gtsam::Pose3();
    ...
    bool Optimize(gtsam::Values* result, std::vector<GraphFactorRecord>* graph_factors) const {
        ...
        if (gnss_factor_count < 2) {
            LOG(ERROR) << "need at least 2 RTK fixed GNSS factors, got " << gnss_factor_count;
            return false;
        }

        const auto pose_noise = [](double rot_sigma, double trans_sigma) {
            return gtsam::noiseModel::Diagonal::Sigmas(
                    (gtsam::Vector6() << rot_sigma, rot_sigma, rot_sigma, trans_sigma, trans_sigma,
                     trans_sigma)
                            .finished());
        };
        ...
        const auto first_prior_noise =
                pose_noise(FLAGS_first_prior_rotation_sigma_rad, FLAGS_first_prior_translation_sigma);
        const auto gnss_position_noise = gtsam::noiseModel::Diagonal::Sigmas(
                (gtsam::Vector3() << FLAGS_gnss_xy_sigma, FLAGS_gnss_xy_sigma, FLAGS_gnss_z_sigma)
                        .finished());
        ...
        for (std::size_t i = 0; i < nodes_.size(); ++i) {
            ...
            if (i > 0) {
                ...
                graph.add(gtsam::BetweenFactor<gtsam::Pose3>(X(i - 1), X(i), relative, lio_noise));
    ...
    bool SaveOutputs(const gtsam::Values& result,
        ...
        for (std::size_t i = 0; i < nodes_.size(); ++i) {
            ...
            FillPoseMessage(local_to_global_ * nodes_[i].gnss_pose, frame.mutable_gnss_pose_3d());
        }

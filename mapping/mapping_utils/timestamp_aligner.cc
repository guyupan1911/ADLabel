...
void TimestampAligner::AddSensorFrameRefs(const std::string& sensor_name,
  ...
  LOG(INFO) << "add sensor: " << sensor_name << ", frames=" << frame_refs.size()
...
std::vector<AlignedFrames> TimestampAligner::Align(
  ...
  for (const auto& main_frame_ref : main_sensor_it->second) {
    ...
    for (auto it = sensor_frame_refs_.begin(); it != sensor_frame_refs_.end();
         ++it) {
      ...
      if (!FindNearest(it->second, main_frame_ref.timestamp_ns,
...
bool TimestampAligner::FindNearest(
    const std::vector<FrameRef>& target_frame_refs,
    int64_t reference_timestamp_ns, FrameRef* matched_frame_ref) const {
  ...
  auto it =
      std::lower_bound(target_frame_refs.begin(), target_frame_refs.end(),
